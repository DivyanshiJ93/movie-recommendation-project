...
def fetch_posters(movie_id):
    response = requests.get(
        'https://api.themoviedb.org/3/movie/{}?api_key=f449f1f4e85cbbfaecacd17539dcd3bc'.format(movie_id))
    data = response.json()
    try:
...
def fetch_person_details(id_):
    data = requests.get(
        'https://api.themoviedb.org/3/person/{}?api_key=f449f1f4e85cbbfaecacd17539dcd3bc'.format(id_)).json()
